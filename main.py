...
from assets.src.serpiente import SERPIENTE
from assets.src.comida import COMIDA
from assets.src.hud import VER_MARCADOR
...
def main() -> None:
    ...
    # ── Sonidos globales ─────────────────────────────────
    sfx_comida  = pygame.mixer.Sound(RUTA_SFX_COMIDA)
    sfx_comida.set_volume(1.0)
    sfx_perdido = pygame.mixer.Sound(RUTA_SFX_PERDIDO)
    ...
    while not termina:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if   event.key == pygame.K_a  and serpiente.DIRECCION != 'D': serpiente.DIRECCION = 'I'
                elif event.key == pygame.K_d and serpiente.DIRECCION != 'I': serpiente.DIRECCION = 'D'
                elif event.key == pygame.K_w    and serpiente.DIRECCION != 'B': serpiente.DIRECCION = 'A'
